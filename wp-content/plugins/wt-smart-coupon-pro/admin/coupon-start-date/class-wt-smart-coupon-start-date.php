...
if( ! class_exists ( 'WT_smart_Coupon_Start_Date' ) ) {
	...
	class WT_smart_Coupon_Start_Date {
        ...
        /**
         * Add fields for entering start date
         * @since 1.2.1
         */
        ...
        /**
         * Check coupon is valid with respecto the start date.
         * @since 1.2.1
         */
        function wt_woocommerce_coupon_is_valid( $valid, $coupon ) {
            ...
            if( $start_date &&  current_time( 'timestamp', true ) < $start_date ) {
                $valid =  false;
                if ( ! $valid ) {
                    throw new Exception( __( 'Sorry, this coupon only available after '.$start_date_date, 'wt-smart-coupons-for-woocommerce-pro' ), 109 );
                }
            }
            return $valid;
